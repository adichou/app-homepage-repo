...
我也是一个摄影爱好者，上面这个问题也同样困扰了我多年。因此我开发了 CamMan 助手 App来解决上面的问题。

# 技术原理

利用手机 + 拓展坞连接外接存储设备，然后通过 CamMan 助手 App 控制外接存储设备的读写，以实现相片的导出和写入。

![CamMan 助手技术原理示意](image/tech-flow.svg)
...
以照片墙形式浏览外置存储和相机卡中的照片与视频：缩略图网格快速总览、点开即看全屏大图、左右滑动连播。支持星级评分与 EXIF 信息查看，评分结果存储在卡上，可以使用 Mac 版的 CamMan 助手（暂未发布)通过电脑进行读取。
